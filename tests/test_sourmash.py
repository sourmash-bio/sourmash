...
def test_do_sourmash_compute_output_and_name_valid_file():
    with utils.TempDirectory() as location:
        ...
        assert (
            data[0]["signatures"][0]["mins"] == data_merged[0]["signatures"][0]["mins"]
        )
...
def test_do_sourmash_compute_10x():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "-k", "31", "--input-is-10x", testdata1],
            in_directory=location,
        )
        ...
        with open(utils.get_test_data("10x-example/barcodes.tsv")) as f:
            true_barcodes = {x.strip() for x in f.readlines()}
...
def test_do_sourmash_compute_multik_input_is_protein():
    with utils.TempDirectory() as location:
        ...
        with open(outfile, "rt") as fp:
            ...
            moltype = {x.minhash.is_molecule_type("protein") for x in siglist}
...
def test_do_compare_quiet():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compare", "short.fa.sig", "short2.fa.sig", "--csv", "xxx", "-q"],
            in_directory=location,
        )
        assert not out
        assert not err
...
def test_do_compare_output_csv():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compare", "short.fa.sig", "short2.fa.sig", "--csv", "xxx"],
            in_directory=location,
        )
...
def test_do_compare_downsample():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "--scaled", "200", "-k", "31", testdata1],
            in_directory=location,
        )
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "--scaled", "100", "-k", "31", testdata2],
            in_directory=location,
        )
...
def test_do_compare_output_multiple_k():
    with utils.TempDirectory() as location:
        ...
        testdata2 = utils.get_test_data("short2.fa")
        status, out, err = utils.runscript(
            "sourmash", ["compute", "-k", "21", testdata1], in_directory=location
        )
        status, out, err = utils.runscript(
            "sourmash", ["compute", "-k", "31", testdata2], in_directory=location
        )
...
def test_do_plot_comparison():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compare", "short.fa.sig", "short2.fa.sig", "-o", "cmp"],
            in_directory=location,
        )

        status, out, err = utils.runscript(
            "sourmash", ["plot", "cmp"], in_directory=location
        )
...
def test_do_plot_comparison_2():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["plot", "cmp", "--pdf"], in_directory=location
        )
...
def test_do_plot_comparison_3():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["plot", "cmp", "--labels"], in_directory=location
        )
...
def test_do_plot_comparison_5_force():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["plot", "cmp", "--labels", "-f"], in_directory=location
        )
        print(status, out, err)
        assert status == 0
...
def test_do_plot_comparison_4_fail_not_distance():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["plot", "cmp", "--labels"], in_directory=location, fail_ok=True
        )
        print(status, out, err)
        assert status != 0
...
def test_plot_subsample_1():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compare"] + inp_sigs + ["-o", "cmp", "-k", "21", "--dna"],
            in_directory=location,
        )

        status, out, err = utils.runscript(
            "sourmash", ["plot", "cmp", "--subsample", "3"], in_directory=location
        )
...
def test_plot_subsample_2():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["plot", "cmp", "--subsample", "3", "--subsample-seed=2"],
            in_directory=location,
        )
...
def test_search_query_sig_does_not_exist():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "short2.fa.sig", "short.fa.sig"],
            in_directory=location,
            fail_ok=True,
        )
...
def test_search_subject_sig_does_not_exist():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
            fail_ok=True,
        )
...
def test_search_second_subject_sig_does_not_exist():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "short.fa.sig", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
            fail_ok=True,
        )
...
def test_search_ignore_abundance():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "-k", "31", "--track-abundance", testdata1, testdata2],
            in_directory=location,
        )

        # Make sure there's different percent matches when using or
        # not using abundance
        status1, out1, err1 = utils.runscript(
            "sourmash",
            ["search", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
        )
        ...
        status2, out2, err2 = utils.runscript(
            "sourmash",
            ["search", "--ignore-abundance", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
        )
...
def test_search_csv():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "short.fa.sig", "short2.fa.sig", "-o", "xxx.csv"],
            in_directory=location,
        )
        print(status, out, err)
...
def test_compare_deduce_molecule():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "-k", "30", "--no-dna", "--protein", testdata1, testdata2],
            in_directory=location,
        )

        status, out, err = utils.runscript(
            "sourmash",
            ["compare", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
        )
        print(status, out, err)
...
def test_compare_choose_molecule_dna():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "-k", "30", "--dna", "--protein", testdata1, testdata2],
            in_directory=location,
        )

        status, out, err = utils.runscript(
            "sourmash",
            ["compare", "--dna", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
        )
        print(status, out, err)
...
def test_compare_choose_molecule_protein():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compare", "--protein", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
        )
        print(status, out, err)
        assert "min similarity in matrix: 0.91" in out
...
def test_compare_deduce_ksize():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "-k", "29", testdata1, testdata2],
            in_directory=location,
        )
...
def test_search_deduce_ksize():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "-k", "23", testdata1, testdata2],
            in_directory=location,
        )
...
def test_do_sourmash_index_multik_fail():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["compute", "-k", "32", testdata2], in_directory=location
        )

        status, out, err = utils.runscript(
            "sourmash",
            ["index", "zzz", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
            fail_ok=True,
        )
...
def test_do_sourmash_index_multimol_fail():
    with utils.TempDirectory() as location:
        ...
        testdata2 = utils.get_test_data("short2.fa")
        status, out, err = utils.runscript(
            "sourmash", ["compute", testdata1], in_directory=location
        )
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "--protein", "-k", "30", testdata2],
            in_directory=location,
        )
...
def test_do_sourmash_index_multiscaled_fail():
    with utils.TempDirectory() as location:
        ...
        testdata2 = utils.get_test_data("short2.fa")
        status, out, err = utils.runscript(
            "sourmash", ["compute", "--scaled", "10", testdata1], in_directory=location
        )
        status, out, err = utils.runscript(
            "sourmash", ["compute", "--scaled", "1", testdata2], in_directory=location
        )

        status, out, err = utils.runscript(
            "sourmash",
            ["index", "-k", "31", "zzz", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
            fail_ok=True,
        )
...
def test_do_sourmash_index_multiscaled_rescale(c):
    ...
    c.run_sourmash(
        "index", "-k", "31", "zzz", "--scaled", "10", "short.fa.sig", "short2.fa.sig"
    )
...
def test_do_sourmash_index_multiscaled_rescale_fail(c):
    ...
    with pytest.raises(ValueError) as e:
        c.run_sourmash(
            "index", "-k", "31", "zzz", "--scaled", "5", "short.fa.sig", "short2.fa.sig"
        )
...
def test_do_sourmash_sbt_search_output():
    with utils.TempDirectory() as location:
        ...
        testdata2 = utils.get_test_data("short2.fa")
        status, out, err = utils.runscript(
            "sourmash", ["compute", testdata1, testdata2], in_directory=location
        )

        status, out, err = utils.runscript(
            "sourmash",
            ["index", "zzz", "-k", "31", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
        )
...
def test_do_sourmash_sbt_search_check_bug():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["index", "zzz", "-k", "31", testdata1, testdata2],
            in_directory=location,
        )
        ...
        status, out, err = utils.runscript(
            "sourmash", ["search", testdata1, "zzz"], in_directory=location
        )
        assert "1 matches:" in out
...
def test_search_deduce_ksize_and_select_appropriate():
    ...
    with utils.TempDirectory() as location:
        ...
        testdata2 = utils.get_test_data("short2.fa")
        status, out, err = utils.runscript(
            "sourmash", ["compute", "-k", "24", testdata1], in_directory=location
        )
        # The DB contains signatres for multiple ksizes
        status, out, err = utils.runscript(
            "sourmash", ["compute", "-k", "23,24", testdata2], in_directory=location
        )
...
def test_search_deduce_ksize_not_unique():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "-k", "23,25", testdata1, testdata2],
            in_directory=location,
        )
...
def test_search_deduce_ksize_vs_user_specified():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "-k", "24", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
            fail_ok=True,
        )
        print(status, out, err)
...
def test_search_containment():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "short.fa.sig", "short2.fa.sig", "--containment"],
            in_directory=location,
        )
        print(status, out, err)
        assert "1 matches" in out
...
def test_search_containment_sbt():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "short.fa.sig", "zzz", "--containment"],
            in_directory=location,
        )
        print(status, out, err)
        assert "1 matches" in out
...
def test_search_gzip():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["search", "zzz.gz", "yyy.gz"], in_directory=location
        )
        print(status, out, err)
        assert "1 matches" in out
        assert "93.0%" in out
...
def test_search_2():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "short.fa.sig", "short2.fa.sig", "short3.fa.sig"],
            in_directory=location,
        )
        print(status, out, err)
...
def test_search_metagenome():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript("sourmash", cmd, in_directory=location)

        assert os.path.exists(os.path.join(location, "gcf_all.sbt.json"))

        cmd = "search {} gcf_all -k 21".format(query_sig)
        status, out, err = utils.runscript(
            "sourmash", cmd.split(" "), in_directory=location
        )
...
def test_search_metagenome_downsample():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", cmd.split(" "), in_directory=location, fail_ok=True
        )
        assert status == -1

        assert "for tree 'gcf_all', scaled value is smaller than query." in err
        assert (
            "tree scaled: 10000; query scaled: 100000. Cannot do similarity search."
            in err
        )
...
def test_search_metagenome_downsample_index(c):
    ...
    c.run_sourmash("index", "gcf_all", "-k", "21", "--scaled", "100000", *testdata_sigs)
...
def test_mash_csv_to_sig():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["import_csv", testdata1, "-o", "xxx.sig"],
            in_directory=location,
        )

        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "-k", "31", "-n", "970", testdata2],
            in_directory=location,
        )

        status, out, err = utils.runscript(
            "sourmash",
            ["search", "-k", "31", "short.fa.sig", "xxx.sig"],
            in_directory=location,
        )
        print(status, out, err)
        assert "1 matches:" in out
...
def test_do_sourmash_sbt_search():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["search", "short.fa.sig", "zzz"], in_directory=location
        )
        print(out)
...
def test_do_sourmash_sbt_search_wrong_ksize():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", testdata1, testdata2, "-k", "31,51"],
            in_directory=location,
        )
...
def test_do_sourmash_sbt_search_multiple():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["search", "short.fa.sig", "zzz", "zzz2"], in_directory=location
        )
        print(out)
...
def test_do_sourmash_sbt_search_and_sigs():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "short.fa.sig", "zzz", "short2.fa.sig"],
            in_directory=location,
        )
        print(out)
...
def test_do_sourmash_sbt_search_downsample():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", testdata1, testdata2, "--scaled=10"],
            in_directory=location,
        )
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", testdata1, "--scaled=5", "-o", "query.sig"],
            in_directory=location,
        )
        ...
        status, out, err = utils.runscript(
            "sourmash", ["search", "query.sig", "zzz"], in_directory=location
        )
        print(out)
...
def test_do_sourmash_sbt_search_downsample_2():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["index", "-k", "31", sbtname, testdata2], in_directory=location
        )
        assert status == 0
...
def test_do_sourmash_sbt_search_selectprot():
    ...
    with utils.TempDirectory() as location:
        ...
        args = ["compute", testdata1, testdata2, "--protein", "--dna", "-k", "30"]
        status, out, err = utils.runscript("sourmash", args, in_directory=location)

        args = ["index", "-k", "31", "zzz", "short.fa.sig", "short2.fa.sig"]
        status, out, err = utils.runscript(
            "sourmash", args, in_directory=location, fail_ok=True
        )
...
def test_do_sourmash_sbt_search_dnaprotquery():
    ...
    with utils.TempDirectory() as location:
        ...
        args = ["index", "zzz", "--protein", "short.fa.sig", "short2.fa.sig"]
        status, out, err = utils.runscript("sourmash", args, in_directory=location)
...
def test_do_sourmash_index_sparseness():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["index", "-k", "31", "zzz", "--traverse-dir", ".", "--sparseness", "1.0"],
            in_directory=location,
        )
...
def test_do_sourmash_sbt_combine():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["index", "-k", "31", "zzz"] + files, in_directory=location
        )
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["sbt_combine", "joined", "zzz.sbt.json", "zzz.sbt.json"],
            in_directory=location,
        )
        ...
        status, out, err = utils.runscript(
            "sourmash", ["search", files[0], "zzz"], in_directory=location
        )
        print(out)
        ...
        status, out, err = utils.runscript(
            "sourmash", ["search", files[0], "joined"], in_directory=location
        )
        print(out)
...
def test_do_sourmash_index_append():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript("sourmash", ["search", sig_loc, sbt_name])
        print(out)
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["index", "-k", "31", "--append", "zzz", "short3.fa.sig"],
            in_directory=location,
        )
        ...
        sig_loc = os.path.join(location, "short3.fa.sig")
        status, out, err = utils.runscript(
            "sourmash", ["search", "--threshold", "0.95", sig_loc, sbt_name]
        )
        print(out)
...
def test_do_sourmash_sbt_search_otherdir():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["index", "-k", "31", "xxx/zzz", "short.fa.sig", "short2.fa.sig"],
            in_directory=location,
        )
...
def test_do_sourmash_sbt_search_scaled_vs_num_1():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["search", sig_loc, sbt_name], fail_ok=True
        )
...
def test_do_sourmash_sbt_search_scaled_vs_num_3():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["search", sig_loc, sig_loc2], fail_ok=True
        )
...
def test_do_sourmash_sbt_search_scaled_vs_num_4():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["search", sig_loc2, sig_loc], fail_ok=True
        )
        assert status == -1
        assert "incompatible - cannot compare" in err
...
def test_do_sourmash_sbt_search_bestonly():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["search", "--best-only", "short.fa.sig", "zzz"],
            in_directory=location,
        )
        print(out)
...
def test_compare_with_abundance_1():
    with utils.TempDirectory() as location:
        ...
        signature.save_signatures([s1], open(os.path.join(location, "e1.sig"), "w"))
        signature.save_signatures([s2], open(os.path.join(location, "e2.sig"), "w"))

        status, out, err = utils.runscript(
            "sourmash", ["search", "e1.sig", "e2.sig", "-k", "5"], in_directory=location
        )
        assert "100.0%" in out
...
def test_gather():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", testdata2, "--scaled", "10", "-o", "query.fa.sig"],
            in_directory=location,
        )
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["gather", "query.fa.sig", "zzz", "-o", "foo.csv", "--threshold-bp=1"],
            in_directory=location,
        )
...
def test_gather_sbt_and_sigs():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "-k", "31", testdata1, testdata2, "--scaled", "10"],
            in_directory=location,
        )
...
def test_gather_file_output():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["gather", "query.fa.sig", "zzz", "--threshold-bp=500", "-o", "foo.out"],
            in_directory=location,
        )
...
def test_gather_metagenome_output_unassigned():
    with utils.TempDirectory() as location:
        ...
        cmd = "gather {} {} {} -k 21".format(
            "unassigned.sig", testdata_sigs, testdata2_sigs
        )
        status, out, err = utils.runscript(
            "sourmash", cmd.split(" "), in_directory=location
        )
        ...
        print(out)
        print(err)
        assert all(("1.3 Mbp       13.6%   28.2%" in out, "NC_011294.1" in out))
...
def test_gather_metagenome_output_unassigned_nomatches(c):
    ...
    c.run_sourmash("gather", query_sig, against_sig, "--output-unassigned", "foo.sig")
...
def test_gather_metagenome_downsample():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["gather", query_sig, "gcf_all", "-k", "21", "--scaled", "100000"],
            in_directory=location,
        )
        ...
        assert "the recovered matches hit 100.0% of the query" in out
        assert all(("5.2 Mbp       32.9%  100.0%" in out, "NC_003198.1" in out))
        assert all(
            ("4.1 Mbp        0.6%    2.4%" in out, "4.1 Mbp        4.4%   17.1%" in out)
        )
...
def test_gather_query_downsample():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["gather", "-k", "31", query_sig] + testdata_sigs,
            in_directory=location,
        )
        ...
        assert all(("4.9 Mbp      100.0%  100.0%" in out, "NC_003197.2" in out))
...
def test_gather_save_matches():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["gather", query_sig, "gcf_all", "-k", "21", "--save-matches", "save.sigs"],
            in_directory=location,
        )
...
def test_gather_error_no_cardinality_query():
    with utils.TempDirectory() as location:
        ...
        testdata3 = utils.get_test_data("short3.fa")
        status, out, err = utils.runscript(
            "sourmash", ["compute", "-k", "31", testdata3], in_directory=location
        )
...
def test_gather_deduce_ksize():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", testdata1, testdata2, "--scaled", "10", "-k", "23"],
            in_directory=location,
        )

        status, out, err = utils.runscript(
            "sourmash",
            ["compute", testdata2, "--scaled", "10", "-k", "23", "-o", "query.fa.sig"],
            in_directory=location,
        )
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["gather", "query.fa.sig", "zzz", "--threshold-bp=1"],
            in_directory=location,
        )
...
def test_gather_abund_1_1():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash", ["gather", query] + against_list, in_directory=location
        )
...
def test_gather_abund_10_1():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["gather", query, "-o", "xxx.csv"] + against_list,
            in_directory=location,
        )
...
def test_gather_abund_10_1_ignore_abundance():
    ...
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["gather", query] + ["--ignore-abundance"] + against_list,
            in_directory=location,
        )
...
def test_sbt_categorize_ignore_abundance():
    with utils.TempDirectory() as location:
        ...
        status2, out2, err2 = utils.runscript("sourmash", args, in_directory=location)
        ...
        status3, out3, err3 = utils.runscript("sourmash", args, in_directory=location)
        ...
        status4, out4, err4 = utils.runscript("sourmash", args, in_directory=location)
    ...
    with utils.TempDirectory() as location:
        ...
        with open(os.path.join(location, "query.fa"), "wt") as fp:
            ...
            for start in range(0, len(record), 100):
                fp.write(
                    ">{}\n{}\n".format(start, record.sequence[start : start + 500])
                )
...
def test_storage_convert():
    ...
    with utils.TempDirectory() as location:
        ...
        shutil.copytree(
            os.path.join(os.path.dirname(testdata), ".sbt.v2"),
        ...
        assert all(
            n1[1].name == n2[1].name
            for (n1, n2) in zip(
                sorted(original.nodes.items()), sorted(ipfs.nodes.items())
            )
        )
        ...
        args = [
            ...
            "convert",
            "-b",
            """'TarStorage("{}")'""".format(os.path.join(location, "v2.sbt.tar.gz")),
            testsbt,
        ]
        status, out, err = utils.runscript("sourmash", args, in_directory=location)
        ...
        assert all(
            n1[1].name == n2[1].name
            for (n1, n2) in zip(
                sorted(original.nodes.items()), sorted(tar.nodes.items())
            )
        )
...
def test_storage_convert_identity():
    with utils.TempDirectory() as location:
        ...
        assert all(
            n1[1].name == n2[1].name
            for (n1, n2) in zip(
                sorted(original.nodes.items()), sorted(identity.nodes.items())
            )
        )
...
def test_storage_convert_fsstorage_newpath():
    with utils.TempDirectory() as location:
        ...
        args = [
            ...
            "convert",
            "-b",
            "fsstorage({})".format(os.path.join(location, "v3")),
            testsbt,
        ]
        status, out, err = utils.runscript("sourmash", args, in_directory=location)
...
def test_migrate():
    with utils.TempDirectory() as location:
        ...
        shutil.copytree(
            os.path.join(os.path.dirname(testdata), ".sbt.v3"),
        ...
        status, out, err = utils.runscript(
            "sourmash", ["migrate", testsbt], in_directory=location
        )
...
def test_license_non_cc0():
    with utils.TempDirectory() as location:
        ...
        status, out, err = utils.runscript(
            "sourmash",
            ["compute", "-k", "31", testdata1, "--license", "GPL"],
            in_directory=location,
            fail_ok=True,
        )
