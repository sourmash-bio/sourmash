...
def test__json_next_signature():
    ...
    it = ijson.parse(io.StringIO(s))
    # no MD5SUM
    sig = _json_next_signature(it, name, filename, ignore_md5sum=True, ijson=ijson)
    ...
    if sys.version_info[0] < 3:
        s = unicode(s)
    it = ijson.parse(io.StringIO(s))
    sig = _json_next_signature(it, name, filename, ignore_md5sum=False, ijson=ijson)
...
def test_load_signaturesset_json_iter():
    ...
    t = list()
    for name, filename in (("Foo", "/tmp/foo"), ("Bar", "/tmp/bar")):
        minhash = (2, 3, 4, 5, 6)
        t.append(
            OrderedDict(
                (
    ...
    # no MD5SUM
    sig_entries = tuple(
        load_signatureset_json_iter(io.StringIO(s), ignore_md5sum=True, ijson=ijson)
    )
