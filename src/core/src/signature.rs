...
impl SigsTrait for Sketch {
    fn size(&self) -> usize {
        match *self {
            Sketch::Draff(ref ukhs) => ukhs.size(),
    ...
    fn to_vec(&self) -> Vec<u64> {
        match *self {
            Sketch::Draff(ref ukhs) => ukhs.to_vec(),
    ...
    fn ksize(&self) -> usize {
        match *self {
            Sketch::Draff(ref ukhs) => ukhs.ksize(),
    ...
    fn seed(&self) -> u64 {
        match *self {
            Sketch::Draff(ref ukhs) => ukhs.seed(),
    ...
    fn hash_function(&self) -> HashFunctions {
        match *self {
            Sketch::Draff(ref ukhs) => ukhs.hash_function(),
    ...
    fn add_hash(&mut self, hash: HashIntoType) {
        match *self {
            Sketch::Draff(ref mut ukhs) => ukhs.add_hash(hash),
    ...
    fn check_compatible(&self, other: &Self) -> Result<(), Error> {
        match *self {
            Sketch::Draff(ref ukhs) => match other {
                Sketch::Draff(ref ot) => ukhs.check_compatible(ot),
