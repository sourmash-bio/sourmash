...
impl std::fmt::Display for ReadingFrame {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let fw_str = String::from_utf8(self.fw.clone()).expect("Invalid UTF-8 sequence in fw");
        if let Some(rc) = &self.rc {
            let rc_str = String::from_utf8(rc.clone()).expect("Invalid UTF-8 sequence in rc");
            write!(f, "Forward: {}, Reverse Complement: {}", fw_str, rc_str)
        } else {
            write!(f, "Forward: {}, Reverse Complement: None", fw_str)
        }
    }
}
...
impl std::fmt::Display for ReadingFrames {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        for (index, frame) in self.0.iter().enumerate() {
            writeln!(f, "Frame {}: {}", index + 1, frame)?;
        }
        Ok(())
    }
}
...
impl ReadingFrames {
    /// Create ReadingFrames based on the input sequence, moltype, and is_protein flag
    pub fn new(sequence: &[u8], is_protein: bool, hash_function: &HashFunctions) -> Self {
        if is_protein {
            ...
            Self::translate_frames(sequence, &dna_rc, dayhoff, hp, hash_function)
            ...
            Self::skipmer_frames(sequence, n, m, hash_function)
        } else {
    ...
    fn skipmer_frames(sequence: &[u8], n: usize, m: usize, hash_function: &HashFunctions) -> Self {
        let frames: Vec<ReadingFrame> = (0..3)
            .map(|frame_number| {
...
impl<'a> Iterator for KmerIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // Validate the k-mer only if the hash function is DNA-based
        ...
        // keeping note from prior implementation :)
...
mod test {
    ...
    //     let expected_kmers = vec![
    //         vec![b"SRR".to_vec(), b"RRA".to_vec()],
    //         vec![b"VVE".to_vec(), b"VE".to_vec()],
    //         vec![b"SSS".to_vec(), b"SS".to_vec()],
    //     ];
