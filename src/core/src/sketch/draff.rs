...
use crate::sketch::hyperloglog::HyperLogLog as HLL;
use crate::sketch::nodegraph::Nodegraph;
use crate::{Error, HashIntoType};
...
impl UKHSTrait for UKHS<u64> {
    ...
    fn merge(&mut self, other: &Self) {
        self.buckets
            ...
            //.for_each(|(c1, c2)| *c1 = (*c1 / 2) + (*c2 / 2))
            //.for_each(|(c1, c2)| *c1 = u64::min(*c1, *c2))
            .for_each(|(c1, c2)| *c1 = u64::max(*c1, *c2))
    }
...
impl SigsTrait for UKHS<u64> {
    ...
    fn add_hash(&mut self, _hash: HashIntoType) {
        unimplemented!()
    }
...
impl UKHSTrait for UKHS<Nodegraph> {
        ...
        W: Write,
    {
        ...
        serde_json::to_writer(writer, &flat)?;
        Ok(())
    }
}
...
impl UKHSTrait for UKHS<HLL> {
    ...
    fn new(ksize: usize, wsize: usize) -> Result<Self, Error> {
        ...
        // TODO: review 0.01 error rate?
        Ok(UKHS {
            ukhs: wk_ukhs,
            buckets: vec![HLL::with_error_rate(0.01, ksize)?; len], // TODO: space usage is 2^b
        })
    }

    fn reset(&mut self) {
        self.buckets = vec![HLL::with_error_rate(0.01, self.ksize()).unwrap(); self.ukhs.len()];
    }
...
impl SigsTrait for UKHS<HLL> {
    ...
    fn add_sequence(&mut self, seq: &[u8], _force: bool) -> Result<(), Error> {
        ...
        //it.into_iter()
        //    .map(|(w_hash, k_hash)| {
        //        self.buckets[self.ukhs.query_bucket(k_hash).unwrap()].add_hash(w_hash);
        //    })
        //    .count();
        ...
        for (w_hash, group) in &it.into_iter().group_by(|(w, _)| *w) {
            let (_, unikmer) = group.min().unwrap();
            self.buckets[self.ukhs.query_bucket(unikmer).unwrap()].add_hash(w_hash);
        }
