...
from . import DEFAULT_SEED, MinHash, create_sbt_index, load_sbt_index
...
def compute(args):
    ...
    def build_siglist(Elist, filename, name=None):
        return [sig.SourmashSignature(E, filename=filename, name=name) for E in Elist]
    ...
    def save_siglist(siglist, output_fp, filename=None):
        ...
        notify(
            "saved {} signature(s). Note: signature license is CC0.".format(
                len(siglist)
            )
        )
    ...
    def maybe_add_alignment(alignment, cell_seqs, args, barcodes):
        ...
        good_barcode = "CB" in alignment.tags and alignment.get_tag("CB") in barcodes
    ...
    if not args.merge:
        ...
        for filename in args.filenames:
            ...
            for n, record in enumerate(screed.open(filename)):
                ...
                add_seq(
                    Elist, record.sequence, args.input_is_protein, args.check_sequence
                )
...
def compare(args):
    ...
    for filename in inp_files:
        ...
        loaded = sig.load_signatures(filename, ksize=args.ksize, select_moltype=moltype)
        loaded = list(loaded)
        if not loaded:
            notify(
...
def plot(args):
    ...
    parser.add_argument("--pdf", action="store_true", help="output PDF, not PNG.")
    parser.add_argument(
        "--labels", action="store_true", help="show sample labels on dendrogram/matrix"
    )
    parser.add_argument(
        "--indices", action="store_false", help="show sample indices but not labels"
    )
    parser.add_argument(
        "--vmax",
        default=1.0,
    ...
    parser.add_argument(
        "--subsample", type=int, help="randomly downsample to this many samples, max."
    )
    parser.add_argument(
        "--subsample-seed",
        type=int,
    ...
    fig = sourmash_fig.plot_composite_matrix(
        D,
        labeltext,
...
def index(args):
    ...
    parser = SourmashArgumentParser()
    parser.add_argument("sbt_name", help="name to save SBT into")
    parser.add_argument("signatures", nargs="+", help="signatures to load into SBT")
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="suppress non-error output"
    )
    parser.add_argument(
    ...
    if args.traverse_directory:
        inp_files = list(sourmash_args.traverse_find_sigs(args.signatures, args.force))
    else:
        inp_files = list(args.signatures)
    ...
    for f in inp_files:
        ...
        siglist = sig.load_signatures(f, ksize=args.ksize, select_moltype=moltype)
...
def search(args):
    ...
    parser.add_argument("databases", help="signatures/SBTs to search", nargs="+")
    parser.add_argument(
        "--traverse-directory",
        action="store_true",
    ...
    parser.add_argument(
        "-n", "--num-results", default=3, type=int, help="number of results to report"
    )
    parser.add_argument(
        "--containment",
        action="store_true",
    ...
    databases = sourmash_args.load_dbs_and_sigs(
        args.databases, query, not args.containment, args.traverse_directory
    )
    ...
    if not args.num_results or n_matches <= args.num_results:
        ...
        print_results("{} matches; showing first {}:", len(results), args.num_results)
    ...
    if args.save_matches:
        ...
        sig.save_signatures([sr.match_sig for sr in results], args.save_matches)


def categorize(args):
    parser = SourmashArgumentParser()
    parser.add_argument("sbt_name", help="name of SBT to load")
    parser.add_argument("queries", nargs="+", help="list of signatures to categorize")
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="suppress non-error output"
    )
    ...
    for queryfile, query, query_moltype, query_ksize in loader:
        ...
        if args.csv:
            w = csv.writer(args.csv)
            w.writerow([queryfile, query.name(), best_hit_query_name, best_hit_sim])
...
def gather(args):
    ...
    parser.add_argument(
        "--scaled", type=float, default=0, help="downsample query to this scaled factor"
    )
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="suppress non-error output"
    )
    ...
    # set up the search databases
    databases = sourmash_args.load_dbs_and_sigs(
        args.databases, query, False, args.traverse_directory
    )
    ...
    print_results(
        "the recovered matches hit {:.1f}% of the query", (1 - weighted_missed) * 100
    )
    print_results("")
    ...
    if args.output_unassigned:
        if not len(query.minhash):
            ...
            sig.save_signatures([sig.SourmashSignature(e)], args.output_unassigned)
...
def multigather(args):
    ...
    query = sourmash_args.load_query_signature(
        args.query[0], ksize=args.ksize, select_moltype=moltype
    )
    # set up the search databases
    databases = sourmash_args.load_dbs_and_sigs(
        args.db, query, False, args.traverse_directory
    )
    ...
    for queryfile in args.query:
        ...
        print_results(
            "the recovered matches hit {:.1f}% of the query",
...
def storage(args):
    ...
    convert_parser.add_argument(
        "-b", "--backend", type=str, help="Backend to convert to"
    )
