...
def load_taxonomy_assignments(
    filename, delimiter=",", start_column=2, use_headers=True, force=False
):
    """
    ...
    if use_headers:
        ...
        for (column, value) in zip(row_headers, first_row):
            ...
            if column.lower() != value.lower():
                notify("** assuming column '{}' is {} in spreadsheet", value, column)
                n_disagree += 1
                if n_disagree > 2:
...
def index(args):
    ...
    p.add_argument("signatures", nargs="+", help="one or more sourmash signatures")
    ...
    notify(
        "{} distinct identities in spreadsheet out of {} rows.",
        len(idx_to_lid),
        num_rows,
    )
    notify(
        "{} distinct lineages in spreadsheet out of {} rows.",
        len(set(idx_to_lid.values())),
        num_rows,
    )
    ...
    if args.traverse_directory:
        ...
        inp_files = list(
            sourmash_args.traverse_find_sigs(
                args.signatures, yield_all_files=yield_all_files
            )
        )
    else:
    ...
    notify(
        "{} assigned lineages out of {} distinct lineages in spreadsheet.",
        len(record_used_lineages),
        len(set(assignments.values())),
    )
    unused_lineages = set(assignments.values()) - record_used_lineages

    notify(
        "{} identifiers used out of {} distinct identifiers in spreadsheet.",
        len(record_used_idents),
        len(set(assignments)),
    )
    ...
    if record_duplicates or record_no_lineage or record_remnants or unused_lineages:
        ...
        if record_no_lineage:
            notify(
                "WARNING: no lineage provided for {} signatures.",
                len(record_no_lineage),
            )
        if record_remnants:
            notify(
                "WARNING: no signatures for {} lineage assignments.",
                len(record_remnants),
            )
        if unused_lineages:
