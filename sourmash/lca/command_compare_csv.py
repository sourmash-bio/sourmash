...
def compare_csv(args):
    ...
    assignments0, num_rows0 = load_taxonomy_assignments(args.csv1, start_column=3)

    notify(
        "loaded {} distinct lineages, {} rows",
        len(set(assignments0.values())),
        num_rows0,
    )
    notify("----")
    ...
    notify(
        "loaded {} distinct lineages, {} rows", len(set(assignments.values())), num_rows
    )
    ...
    notify(
        "{} total assignments, {} differ between spreadsheets.", n_total, n_different
    )
    notify("{} are compatible (one lineage is ancestor of another.", n_compat)
    notify("{} are incompatible (there is a disagreement in the trees).", n_incompat)
