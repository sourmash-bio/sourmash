...
class MinHash(RustObject):
    def __init__(
        ...
        self._objptr = lib.kmerminhash_new(
            n, ksize, is_protein, dayhoff, seed, int(max_hash), track_abundance
        )
    ...
    def __setstate__(self, tup):
        (n, ksize, is_protein, dayhoff, mins, _, track_abundance, max_hash, seed) = tup

        self.__del__()
        self._objptr = lib.kmerminhash_new(
            n, ksize, is_protein, dayhoff, seed, max_hash, track_abundance
        )
        if track_abundance:
            self.set_abundances(mins)
        else:
    ...
    def downsample_n(self, new_num):
        ...
        a = MinHash(
            new_num, self.ksize, self.is_protein, self.dayhoff, self.track_abundance, self.seed, 0
        )
